...
def create_office_chair(x, y, target_x=None, target_y=None, rot=None, index=""):
    """創建辦公椅 - 修正朝向計算"""
    # 計算旋轉角度（修正：atan2(dy, dx) 不是 atan2(dx, dy)）
    if rot is None and target_x is not None and target_y is not None:
        dx = target_x - x
        dy = target_y - y
        rot = math.atan2(dy, dx)  # 修正！原本寫反了
    elif rot is None:
        rot = 0
...
# ========== 工作區（北區）==========
# 注意：玻璃牆門在 y=1.5，門在 x=0
# 工作站 3 原本在 (0, 5) 會擋住門口，移到 (-2, 5)
create_workstation(-5.5, 4, facing='east', index=1)
create_workstation(-5.5, 6, facing='east', index=2)
create_workstation(-2, 5, facing='north', index=3)  # 移到左邊，不擋門
...
bpy.ops.export_scene.gltf(
    filepath='/mnt/e_drive/claude-office/blender/exports/office_v40.glb',
    export_format='GLB',
    use_selection=True
)

print("已導出: /mnt/e_drive/claude-office/blender/exports/office_v40.glb")
