...
create_chair(-5.5, 4 - 0.875, rot=0, index="_WS1")

# 工作站 2: 左側，面向東
create_desk(-5.5, 6, rot=math.pi/2, index=2)
create_chair(-5.5, 6 - 0.875, rot=0, index="_WS2")

# 工作站 3: 中間，面向北
create_desk(0, 5, rot=0, index=3)
create_chair(0, 5 - 0.875, rot=math.pi, index="_WS3")

# 工作站 4: 右側，面向西
create_desk(5.5, 4, rot=-math.pi/2, index=4)
create_chair(5.5, 4 + 0.875, rot=math.pi, index="_WS4")

# 工作站 5: 右側，面向西
create_desk(5.5, 6, rot=-math.pi/2, index=5)
create_chair(5.5, 6 + 0.875, rot=math.pi, index="_WS5")

# ========== 導出 ==========
bpy.ops.object.select_all(action='SELECT')
bpy.ops.export_scene.gltf(
    filepath='/mnt/e_drive/claude-office/blender/exports/office_v48.glb',
    export_format='GLB',
    use_selection=True
)
