#!/usr/bin/env python3
"""
Claude Office V26 - 簡化 PBR 紋理版
使用簡單的紋理設置，確保 GLTF 導出正確
"""
...
def create_texture_material(name, color_path=None, roughness=0.5, metallic=0.0):
    """
    創建帶紋理的簡化材質（GLTF 兼容）
    只使用 Base Color 紋理
    """
...
# 地板 - 木地板紋理
mat_floor = create_texture_material(
    "WoodFloor",
    color_path=f"{TEXTURE_DIR}/wood_diffuse.jpg",
    roughness=0.8
)
print("  ✓ 木地板紋理")

# 椅座/沙發 - 布料紋理
mat_seat = create_texture_material(
    "Fabric",
    color_path=f"{TEXTURE_DIR}/fabric_diffuse.jpg",
    roughness=0.9
)
print("  ✓ 布料紋理")
