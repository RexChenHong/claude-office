#!/usr/bin/env python3
"""V46 - 修正椅背 sin/cos 互換錯誤
1. 椅背位置計算：sin/cos 互換，讓椅背真正在座墊「後方」
2. 移除玻璃隔間
"""
import bpy
import math
...
def create_office_chair(x, y, target_x=None, target_y=None, rot=None, index=""):
    """創建辦公椅 - L 型結構
    V46 修正：sin/cos 互換，讓椅背真正在座墊「後方」
    
    座墊面向 rot 方向，椅背應在 -rot 方向（後方）
    當 rot=0（面向 Y+），椅背應在 Y- 方向
    當 rot=π/2（面向 X+），椅背應在 X- 方向
    """
    # 計算旋轉角度（椅子面向目標）
    ...
    # 椅背（在座墊後方）- V46 修正
    # 座墊半深度 = 0.38/2 = 0.19
    # back_offset = 0.25，椅背中心在座墊後方 0.25 處
    # 椅背前緣 = 0.25 - 0.06 = 0.19 = 座墊後緣 ✓
    # 
    # 關鍵修正：sin/cos 互換
    # 當 rot=0（面向 Y+）時，椅背應在 Y- 方向
    #   back_x = x - 0.25 * sin(0) = x ✓
    #   back_y = y - 0.25 * cos(0) = y - 0.25 ✓
    # 當 rot=π/2（面向 X+）時，椅背應在 X- 方向
    #   back_x = x - 0.25 * sin(π/2) = x - 0.25 ✓
    #   back_y = y - 0.25 * cos(π/2) = y ✓
    
    back_offset = 0.25  # 椅背中心到椅子中心的距離
    back_x = x - back_offset * math.sin(rot)  # V46: sin/cos 互換
    back_y = y - back_offset * math.cos(rot)  # V46: sin/cos 互換
    
    # 椅背尺寸
    back_width = 0.36   # X 軸寬度
    back_thickness = 0.06  # Y 軸厚度（減薄）
    back_height = 0.5    # Z 軸高度
    
    bpy.ops.mesh.primitive_cube_add(size=1)
    back = bpy.context.active_object
    back.scale = (back_width, back_thickness, back_height)
    
    # 椅背 Z 位置：底部在座墊頂部之上
    # 座墊頂部 z = 0.47 + 0.08/2 = 0.51
    # 椅背底部 z = 0.51，椅背中心 z = 0.51 + 0.5/2 = 0.76
    back_z = 0.51 + back_height / 2
...
bpy.ops.export_scene.gltf(
    filepath='/mnt/e_drive/claude-office/blender/exports/office_v46.glb',
    export_format='GLB',
    use_selection=True
)

print("已導出: /mnt/e_drive/claude-office/blender/exports/office_v46.glb")
