#!/usr/bin/env python3
"""V44 - 修正椅子椅背、移除玻璃隔間
1. 椅子：back_offset 從 0.25 改為 0.38，確保椅背完全在座墊後方（L 型）
2. 移除中間的玻璃隔間牆
"""
import bpy
import math
...
def create_office_chair(x, y, target_x=None, target_y=None, rot=None, index=""):
    """創建辦公椅 - L 型結構
    椅背在座墊後方，向後傾斜
    V44 修正：back_offset 從 0.25 改為 0.38
    """
    # 計算旋轉角度（椅子面向目標）
    ...
    # 椅背（在座墊後方）- V44 修正
    # 座墊半深度 = 0.38/2 = 0.19
    # back_offset = 0.38，椅背中心在座墊後方 0.38 處
    # 椅背前緣 = 0.38 - 0.12/2 = 0.32 > 座墊半深度 0.19 ✓
    back_offset = 0.38  # V44: 從 0.25 改為 0.38
    ...
    # 椅背中心高度 - V45 修正：椅背底部應在座墊頂部之上
    # 座墊頂部 z = 0.47 + 0.08/2 = 0.51
    # 椅背底部應 >= 0.51，所以椅背中心 >= 0.51 + back_height/2
    back_z = 0.51 + back_height / 2  # 椅背底部剛好在座墊頂部
...
# ========== V44: 移除玻璃隔間 ========== 
# (原本的 create_glass_partition_with_door 已移除)
...
bpy.ops.export_scene.gltf(
    filepath='/mnt/e_drive/claude-office/blender/exports/office_v44.glb',
    export_format='GLB',
    use_selection=True
)

print("已導出: /mnt/e_drive/claude-office/blender/exports/office_v44.glb")
