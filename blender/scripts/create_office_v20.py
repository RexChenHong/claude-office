#!/usr/bin/env python3
"""
Claude Office V20 - 寫實風格
使用真實 PBR 紋理（直接映射，不用程序化節點）
"""
...
# 載入木地板紋理
wood_diffuse = load_texture("Wood_Diffuse", TEX_PATH + "wood_diffuse.jpg")
wood_normal = load_texture("Wood_Normal", TEX_PATH + "wood_normal.jpg")
wood_roughness = load_texture("Wood_Roughness", TEX_PATH + "wood_roughness.jpg")
wood_ao = load_texture("Wood_AO", TEX_PATH + "wood_ao.jpg")

# 載入布料紋理
fabric_diffuse = load_texture("Fabric_Diffuse", TEX_PATH + "fabric_diffuse.jpg")
fabric_normal = load_texture("Fabric_Normal", TEX_PATH + "fabric_normal.jpg")
fabric_roughness = load_texture("Fabric_Roughness", TEX_PATH + "fabric_roughness.jpg")

# ========== 材質（使用紋理）==========

def create_textured_material(name, diffuse_img, normal_img=None, roughness_img=None, color_tint=(1, 1, 1, 1)):
    """創建帶紋理的材質（GLTF 兼容）"""
    ...
    mapping.inputs['Scale'].default_value = (6, 6, 1)  # 調整紋理大小
    ...
    if diffuse_img:
        ...
        # 顏色調整（可選）
        if color_tint != (1, 1, 1, 1):
            # 使用 Bright/Contrast 節點調暗
            bright = nodes.new('ShaderNodeBrightContrast')
            bright.location = (-200, 200)
            bright.inputs['Bright'].default_value = -0.3  # 變暗
            bright.inputs['Contrast'].default_value = 0.2  # 增加對比

            links.new(diffuse_tex.outputs['Color'], bright.inputs['Color'])
            links.new(bright.outputs['Color'], principled.inputs['Base Color'])
        else:
    ...
    if roughness_img:
        ...
        roughness_img.colorspace_settings.name = 'Linear'
...
# 創建材質
mat_floor = create_textured_material("WoodFloor", wood_diffuse, wood_normal, wood_roughness, color_tint=(0.7, 0.6, 0.5, 1))
mat_wall = create_simple_material("Wall", (0.92, 0.92, 0.9, 1), roughness=0.85)
mat_glass = create_simple_material("Glass", (0.85, 0.92, 0.98, 0.15), roughness=0.05)
mat_metal_frame = create_simple_material("Metal_Frame", (0.08, 0.08, 0.08, 1), roughness=0.2, metallic=0.85)
mat_desktop = create_simple_material("Desktop", (0.95, 0.95, 0.93, 1), roughness=0.25)
mat_seat = create_textured_material("Seat", fabric_diffuse, fabric_normal, fabric_roughness, color_tint=(0.3, 0.3, 0.3, 1))
mat_metal = create_simple_material("Metal", (0.35, 0.35, 0.37, 1), roughness=0.15, metallic=0.9)
mat_plastic = create_simple_material("Plastic", (0.04, 0.04, 0.04, 1), roughness=0.35)
mat_screen = create_simple_material("Screen", (0.1, 0.13, 0.16, 1), roughness=0.08)
mat_meeting_table = create_simple_material("Meeting_Table", (0.55, 0.45, 0.38, 1), roughness=0.35)
mat_whiteboard = create_simple_material("Whiteboard", (0.98, 0.98, 0.98, 1), roughness=0.25)
mat_sofa = create_textured_material("Sofa", fabric_diffuse, fabric_normal, fabric_roughness, color_tint=(0.4, 0.4, 0.42, 1))
mat_plant_pot = create_simple_material("Pot", (0.65, 0.45, 0.32, 1), roughness=0.6)
mat_plant = create_simple_material("Plant", (0.18, 0.38, 0.15, 1), roughness=0.5)
mat_printer = create_simple_material("Printer", (0.18, 0.18, 0.18, 1), roughness=0.3)
mat_water_dispenser = create_simple_material("WaterDispenser", (0.88, 0.88, 0.88, 1), roughness=0.2)
mat_cabinet = create_simple_material("Cabinet", (0.55, 0.55, 0.57, 1), roughness=0.3, metallic=0.4)
mat_trash_bin = create_simple_material("TrashBin", (0.22, 0.22, 0.22, 1), roughness=0.5)
mat_carpet = create_simple_material("Carpet", (0.5, 0.43, 0.38, 1), roughness=0.95)
mat_lamp = create_simple_material("Lamp", (0.12, 0.12, 0.12, 1), roughness=0.2, metallic=0.7)
