#!/usr/bin/env python3
"""
Claude Office V24 - PBR 紋理版
為地板、桌面、椅子應用真實 PBR 紋理
"""
...
def create_pbr_material(name, diffuse_path=None, normal_path=None, roughness_path=None, 
...
mat_floor = create_pbr_material(
    "DarkWoodFloor_PBR",
    diffuse_path=f"{TEXTURE_DIR}/wood_diffuse.jpg",
    normal_path=f"{TEXTURE_DIR}/wood_normal.jpg",
    roughness_path=f"{TEXTURE_DIR}/wood_roughness.jpg",
    base_color=(0.25, 0.18, 0.12, 1),
    roughness=0.85
)

# 椅座 - 布料 PBR
mat_seat = create_pbr_material(
    "Seat_Fabric_PBR",
    diffuse_path=f"{TEXTURE_DIR}/fabric_diffuse.jpg",
    normal_path=f"{TEXTURE_DIR}/fabric_normal.jpg",
    roughness_path=f"{TEXTURE_DIR}/fabric_roughness.jpg",
    base_color=(0.06, 0.06, 0.06, 1),
    roughness=0.9
)

# 沙發 - 布料 PBR（灰色）
mat_sofa = create_pbr_material(
    "Sofa_Fabric_PBR",
    diffuse_path=f"{TEXTURE_DIR}/fabric_diffuse.jpg",
    normal_path=f"{TEXTURE_DIR}/fabric_normal.jpg",
    roughness_path=f"{TEXTURE_DIR}/fabric_roughness.jpg",
    base_color=(0.22, 0.22, 0.24, 1),
    roughness=0.95
)
...
add_bevel(s, segments=5, width=0.05)  # 沙發用更大圓角
...
print("紋理：wood_diffuse + normal + roughness, fabric_diffuse + normal + roughness")
