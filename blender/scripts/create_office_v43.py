#!/usr/bin/env python3
"""V43 - 修正椅子椅背、移除門框
1. 椅子：椅背增加厚度，確保形成 L 型（椅背在座墊後方，不是橫跨在座墊上）
2. 玻璃隔間門：移除頂部門框（多餘的橫條）
3. 休息區：保留 L 型沙發
"""
import bpy
import math
...
def create_office_chair(x, y, target_x=None, target_y=None, rot=None, index=""):
    ...
    # 椅背（在座墊後方，向後傾斜）
    # 關鍵修正：增加椅背的厚度（Y 軸），讓它看起來是在座墊"後方"
    back_offset = 0.25  # 椅背距離椅子中心的偏移（增加）
    back_x = x - back_offset * math.cos(rot)
    back_y = y - back_offset * math.sin(rot)
    
    # 椅背尺寸：寬度略小於座墊，厚度增加（Y 軸），高度 0.5m
    # scale = (X寬, Y厚, Z高)
    back_width = 0.36
    back_thickness = 0.12  # 增加厚度（從 0.06 改為 0.12）
    back_height = 0.5
    ...
    # 椅背中心位置（考慮傾斜）
    # 座墊頂部在 z=0.51，椅背底部應該在 z=0.51 附近
    # 椅背中心高度 = 座墊頂部 + 椅背半高 - 重疊部分
    back_z = 0.51 + back_height / 2 - 0.05  # 稍微重疊
...
bpy.ops.export_scene.gltf(
    filepath='/mnt/e_drive/claude-office/blender/exports/office_v43.glb',
    export_format='GLB',
    use_selection=True
)

print("已導出: /mnt/e_drive/claude-office/blender/exports/office_v43.glb")
