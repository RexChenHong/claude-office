...
1. Node translation（骨骼默認位置）
2. InverseBindMatrix（蒙皮矩陣）
"""
import struct
import json
import copy
...
print(f"找到 {len(bone_translations)} 個骨骼 translation")
print(f"原始 IBM 數據: {orig_ibm_count} 個矩陣, {len(orig_ibm_data)} bytes")
...
print(f"修復了 {fixed_translations} 個骨骼 translation")

# 修復 inverseBindMatrix
# 直接用原始模型的 IBM 替換
target_skin = target_gltf['skins'][0]
target_joints = target_skin['joints']
...
# 寫入修復後的文件
output = '/mnt/e_drive/claude-office/blender/exports/character_walk_fixed_v3.glb'
