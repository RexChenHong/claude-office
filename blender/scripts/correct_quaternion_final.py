...
for frame in range(0, 121, 10):
    ...
    if 'spine2' in bones:
        # 創建 X 軸旋轉的四元數
        angle = 0.03 * math.sin(t * 2 * math.pi)
        q = Quaternion((math.cos(angle/2), math.sin(angle/2), 0, 0))
        bones['spine2'].rotation_quaternion = q
        bones['spine2'].keyframe_insert(data_path="rotation_quaternion", frame=frame, group="Idle")

    # 輕微上下移動
    if 'hip' in bones:
        bones['hip'].location = Vector((0, 0.02 * math.sin(t * 2 * math.pi), 0))
...
# 導出
output = '/mnt/e_drive/claude-office/blender/exports/character_correct_quaternion.glb'
print(f"\n開始導出到: {output}")
