#!/usr/bin/env python3
"""
Claude Office V18 - 真實 PBR 紋理
使用 Poly Haven CC0 紋理
"""
...
def create_pbr_material(name, diffuse_img, normal_img=None, roughness_img=None, ao_img=None, color_multiplier=(1, 1, 1, 1), roughness_base=0.5):
    """創建 PBR 材質"""
    ...
    mapping.inputs['Scale'].default_value = (8, 8, 1)  # 調整紋理縮放
    ...
    if diffuse_img:
        ...
        # 混合顏色（可選）
        if color_multiplier != (1, 1, 1, 1):
            mix_rgb = nodes.new('ShaderNodeMixRGB')
            mix_rgb.location = (-200, 200)
            mix_rgb.blend_type = 'MULTIPLY'  # 使用 Multiply 模式調暗
            mix_rgb.inputs['Color2'].default_value = color_multiplier
            mix_rgb.inputs['Fac'].default_value = 1.0  # 完全應用
            links.new(diffuse_tex.outputs['Color'], mix_rgb.inputs['Color1'])
        ...
        # 連接 Alpha（如果有）
        if diffuse_img.alpha_mode != 'NONE':
            links.new(diffuse_tex.outputs['Alpha'], principled.inputs['Alpha'])
    else:
        # 沒有紋理，使用純色
        principled.inputs['Base Color'].default_value = color_multiplier
    ...
    if roughness_img:
        ...
        mix_rough.location = (-200, -300)
        mix_rough.inputs['Color2'].default_value = (roughness_base, roughness_base, roughness_base, 1)
        mix_rough.inputs['Fac'].default_value = 0.5

        links.new(roughness_tex.outputs['Color'], mix_rough.inputs['Color1'])
        links.new(mix_rough.outputs['Color'], principled.inputs['Roughness'])
    else:
        principled.inputs['Roughness'].default_value = roughness_base

    # AO 紋理
    if ao_img:
        ao_tex = nodes.new('ShaderNodeTexImage')
        ao_tex.location = (-400, -500)
        ao_img.colorspace_settings.name = 'Linear'
        ao_tex.image = ao_img
        ao_tex.label = "AO"
        ...
        mix_ao.inputs['Color2'].default_value = (0.9, 0.9, 0.9, 1)
        mix_ao.inputs['Fac'].default_value = 0.3

        links.new(ao_tex.outputs['Color'], mix_ao.inputs['Color1'])
    ...
    # 連接映射
    if diffuse_img or normal_img or roughness_img or ao_img:
...
def create_metal_material():
    ...
    principled.inputs['Base Color'].default_value = (0.12, 0.12, 0.12, 1)
...
# 創建所有材質
mat_floor = create_pbr_material("WoodFloor_PBR", wood_diffuse, wood_normal, wood_roughness, wood_ao, color_multiplier=(0.6, 0.45, 0.35, 1), roughness_base=0.35)
...
mat_seat = create_pbr_material("Fabric_PBR", fabric_diffuse, fabric_normal, fabric_roughness, color_multiplier=(0.08, 0.08, 0.08, 1), roughness_base=0.85)
...
mat_sofa = create_pbr_material("Sofa_PBR", fabric_diffuse, fabric_normal, fabric_roughness, color_multiplier=(0.28, 0.28, 0.3, 1), roughness_base=0.9)
